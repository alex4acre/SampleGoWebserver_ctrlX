...
function BackupRecipe(){
    window.alert("Writing the recipes...");
    RestBackupRecipe.open("PUT", "https://" + strIPAddress + "/automation/api/v1/plc/app/Application/sym/USBAccess/bCopyRecipeToUSB", true); 
	RestBackupRecipe.setRequestHeader('Content-type', 'application/json');
	RestBackupRecipe.setRequestHeader('Authorization', "Bearer " + xToken);
	RestBackupRecipe.responseType = 'json';
	let json = JSON.stringify({"value":true ,"type":"bool8"});
	RestBackupRecipe.send(json);
}

function RestoreRecipe(){
    window.alert("Restoring the recipes...");
    RestRestoreRecipe.open("PUT", "https://" + strIPAddress + "/automation/api/v1/plc/app/Application/sym/USBAccess/bCopyRecipeFromUSB", true); 
	RestRestoreRecipe.setRequestHeader('Content-type', 'application/json');
	RestRestoreRecipe.setRequestHeader('Authorization', "Bearer " + xToken);
	RestRestoreRecipe.responseType = 'json';
	let json = JSON.stringify({"value":true ,"type":"bool8"});
	RestRestoreRecipe.send(json);
}

function BackupApp(){
    window.alert("Backing up the application...");
    RestBackupApp.open("PUT", "https://" + strIPAddress + "/automation/api/v1/plc/app/Application/sym/USBAccess/bCopyApplicationToUSB", true); 
	RestBackupApp.setRequestHeader('Content-type', 'application/json');
	RestBackupApp.setRequestHeader('Authorization', "Bearer " + xToken);
	RestBackupApp.responseType = 'json';
	let json = JSON.stringify({"value":true ,"type":"bool8"});
	RestBackupApp.send(json);
}

function RestoreApp(){
    window.alert("Restoring the Application...");
    RestRestoreApp.open("PUT", "https://" + strIPAddress + "/automation/api/v1/plc/app/Application/sym/USBAccess/bCopyApplicationFromUSB", true); 
	RestRestoreApp.setRequestHeader('Content-type', 'application/json');
	RestRestoreApp.setRequestHeader('Authorization', "Bearer " + xToken);
	RestRestoreApp.responseType = 'json';
	let json = JSON.stringify({"value":true ,"type":"bool8"});
	RestRestoreApp.send(json);
}

//get the USB ID to mount
...
RestGetUSBID_Mount.onload = function() {
    ...
    window.alert("Mounted: " + myObj.response[0].device);
    ...
    let json = JSON.stringify({"action": "mount","parameters": {"media": myObj.response[0].uuid, "assignment":"data-exchange"}});
    RestMountUSB.send(json);
  }

RestMountUSB.onload  = function() {
    WriteUSBPath(myObj.response[0].device)
  }

//get USB ID to unmount
...
function WriteUSBPath(val){
	RestUSBPath.open("PUT", "https://" + strIPAddress + "/automation/api/v1/plc/app/Application/sym/USBAccess/usbMediaName", true); 
	RestUSBPath.setRequestHeader('Content-type', 'application/json');
	RestUSBPath.setRequestHeader('Authorization', "Bearer " + xToken);
    let json = JSON.stringify({"value":val ,"type":"string"});
	RestUSBPath.responseType = 'json';
	RestUSBPath.send(json);
    }
...
function FC_Connect() {
    ...
    strIPAddress = location.hostname;
	document.title = "Controller: " + location.hostname
